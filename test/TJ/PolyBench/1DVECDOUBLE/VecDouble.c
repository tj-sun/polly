...
{
	// Load the kernel source code into the array source_str
	fp = fopen("2DConvolution.spir", "r");
	if (!fp) {
		fprintf(stdout, "Failed to load SPIR kernel.\n");
		exit(1);
	}
	...
	spir_size = ftell(fp);
	rewind(fp);
	spir_str = new unsigned char[spir_size];
	fread(spir_str, 1, spir_size, fp);
	fclose( fp );
}
...
void cl_load_prog()
{
	...
	// Create a program from SPIR binary
	clProgram = clCreateProgramWithBinary(clGPUContext, 1, &device_id, &spir_size, (const unsigned char**)&spir_str, &spir_status, &errcode);
	delete [] spir_str;
